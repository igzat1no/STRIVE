...
def get_pointcloud_from_depth(rgb: np.ndarray, depth: np.ndarray, intrinsic: np.ndarray):
    ...
    pixel_z = (depth.shape[0] - 1 - filter_z -
               intrinsic[1][2]) * depth_values / intrinsic[1][1]
...
def get_pointcloud_from_depth_mask(depth: np.ndarray,
                                   mask: np.ndarray,
    ...
    if coord_maps is not None:
        new_coords = coord_maps[filter_z, filter_x]  # 获取新的坐标
        filter_z = new_coords[:, 0]  # 取出新的 X 坐标
        filter_x = new_coords[:, 1]  # 取出新的 Y 坐标
        if depth_map is not None:
            # round to int
            filter_z = np.round(filter_z)
            filter_x = np.round(filter_x)
            depth_values = depth_map[filter_z.astype(int), filter_x.astype(int)]

    # only keep the depth values that are not zero
    filter_z = filter_z[depth_values > 0]
    filter_x = filter_x[depth_values > 0]
...
def project_to_camera(pcd, intrinsic, position, rotation):
    ...
    try:
        camera_points = np.concatenate(
            (pcd.point.positions.cpu().numpy(), np.ones(
                (pcd.point.positions.shape[0], 1))),
            axis=-1)
    except:
        camera_points = np.concatenate(
            (pcd.points, np.ones((np.array(pcd.points).shape[0], 1))), axis=-1)
    camera_points = np.matmul(extrinsic, camera_points.T).T[:, 0:3]
    depth_values = -camera_points[:, 2]
    flag = depth_values > 0
    camera_points = camera_points[flag]
    depth_values = depth_values[flag]
    filter_x = (camera_points[:, 0] * intrinsic[0][0] / depth_values +
                intrinsic[0][2]).astype(np.int32)
    filter_z = (-camera_points[:, 1] * intrinsic[1][1] / depth_values - intrinsic[1][2] +
                intrinsic[1][2] * 2 - 1).astype(np.int32)
...
def gpu_pointcloud_from_array(points, colors, device):
    ...
    pointcloud.point.positions = o3d.core.Tensor(points,
                                                 dtype=o3d.core.Dtype.Float32,
                                                 device=device)
    pointcloud.point.colors = o3d.core.Tensor(colors.astype(np.float32) / 255.0,
                                              dtype=o3d.core.Dtype.Float32,
                                              device=device)
    return pointcloud
...
def gpu_pointcloud(pointcloud, device):
    ...
    new_pointcloud.point.positions = o3d.core.Tensor(np.asarray(pointcloud.points),
                                                     device=device)
    new_pointcloud.point.colors = o3d.core.Tensor(np.asarray(pointcloud.colors),
                                                  device=device)
...
def cpu_pointcloud(pointcloud):
    ...
    new_pointcloud.points = o3d.utility.Vector3dVector(
        pointcloud.point.positions.cpu().numpy())
    new_pointcloud.colors = o3d.utility.Vector3dVector(
        pointcloud.point.colors.cpu().numpy())
    return new_pointcloud
...
def gpu_merge_pointcloud(pcdA, pcdB, merge_color=True):
    ...
    if pcdA is None and pcdB is None:
        ...
        empty_pc.point.positions = o3d.core.Tensor([],
                                                   dtype=o3d.core.Dtype.Float32,
                                                   device=device)
        empty_pc.point.colors = o3d.core.Tensor([],
                                                dtype=o3d.core.Dtype.Float32,
                                                device=device)
        return empty_pc
    ...
    if merge_color:
        ...
        pcdB.point.colors = o3d.core.Tensor(colorB,
                                            dtype=pcdA.point.colors.dtype,
                                            device=pcdA.point.colors.device)
    return pcdA + pcdB


def gpu_cluster_filter(pointcloud, eps=0.5, min_points=10):
    labels = pointcloud.cluster_dbscan(eps=eps,
...
def cpu_cluster_filter(pointcloud, eps=0.5, min_points=10):
    labels = pointcloud.cluster_dbscan(eps=eps,
                                       min_points=min_points,
                                       print_progress=False)
